...
flags:
    ...
    python -m pytest ./projects/lsst_y1/tests --high=1
...
pytest fixtures as arguments. An environment variable is also how the
suite already takes its other run-time switch (COCOA_ACCURACY_NMODELS
in test_accuracy.py), and it lets a direct
`python test_fastpt.py` run set the same switch by exporting the
variable by hand.
"""
...
def pytest_addoption(parser):
    ...
    --high=0 (the default) runs the CFASTPT-vs-FASTPT sweep (test 15)
    at the frozen default settings; --high=1 repeats it with the
    HIGH_ACCURACY settings of the low-vs-high accuracy checks applied
    to both implementations. The full comparison is both invocations,
...
def pytest_configure(config):
    ...
    lands in the COCOA_FASTPT_HIGH environment variable (see the
    module docstring for why an environment variable), which
